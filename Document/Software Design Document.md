...
3. 이슈 생성 및 수정 시 본문 언어를 Markdown 또는 HTML 중에서 선택할 수 있어야 하며, 체크리스트, 코드 블록, 첨부, 참조 링크를 포함한 GitHub식 작성 흐름을 지원해야 한다. 저장 계층은 본문 원문과 본문 포맷을 함께 보존해 렌더링과 내보내기 단계에서 사용자의 작성 의도를 잃지 않아야 한다. 로컬 데스크톱 앱의 읽기 뷰는 WebView 기반 HTML 렌더링을 사용해 HTML fragment, 전체 HTML 문서, inline style, `<style>` 기반 CSS를 실제 브라우저에 가깝게 재현해야 한다.
...
4. 전역 우측 컨텍스트 패널은 이슈 상세와 편집의 기본 위치로 사용하지 않는다. 이슈를 선택하면 GitHub Issues처럼 별도 상세 상태로 전환하고, 제목, 본문, 댓글, 타임라인, 메타데이터 편집을 상세 화면 내부에 배치한다.
5. 좌측 내비게이션은 모든 테마에서 hover와 active 상태의 명도 대비가 충분해야 하며, 배경과 유사한 색으로 변해 항목을 읽기 어려워지는 상태를 허용하지 않는다.
6. 시각 시스템: 절제된 중성 계열 색상, 명확한 타이포그래피 위계, 얇고 정돈된 구분선, 과하지 않은 애니메이션을 기본 방향으로 삼는다.
...
5. 목록 화면은 앱 진입 시 첫 이슈를 자동으로 열지 않고, 사용자가 특정 이슈 행을 선택했을 때 별도 이슈 상세 화면으로 전환한다.
6. 빠른 메타데이터 편집을 위한 인라인 액션을 지원한다.
7. overdue 이슈, 오늘 마감 이슈, 곧 예정된 항목을 빠르게 식별할 수 있는 시각적 표시를 제공한다.
...
3. 담당자, 라벨, 프로젝트, 마일스톤, 우선순위, 닫힘 사유, 연결 이슈 정보는 전역 우측 패널이 아니라 상세 화면 내부의 메타데이터 편집 영역에서 제공한다.
4. 상세 화면은 목록 안의 단일 프레임 패널이 아니라 목록 화면과 분리된 메인 콘텐츠 상태여야 하며, `Back to issues` 동작으로 목록과 선택 상태를 명확하게 되돌릴 수 있어야 한다.
5. 사용자 행동은 "읽기", "기록", "메타데이터 수정"이 같은 화면에서 자연스럽게 이어져야 한다.
6. 이슈 설명과 댓글 작성란은 Markdown/HTML 포맷 선택을 제공해야 하며, 상세 화면에는 저장된 포맷을 사용자가 즉시 확인할 수 있는 언어 배지를 표시한다.
7. GitHub 개별 이슈 화면의 세부 동작은 상세 화면 구현과 관련 회귀 테스트를 기준으로 유지한다.
...
3. GitHub식 목록-상세 전환 상태와 이슈 상세 타임라인 화면
4. 프로젝트 목록
5. 프로젝트 상세 보드 뷰
6. 프로젝트 상세 테이블 뷰
7. 프로젝트 상세 캘린더 뷰
...
| Comment | 이슈 대화 및 기록. 본문 원문과 Markdown/HTML 포맷 정보를 함께 보존한다. |
| Label | 분류용 태그 |
...
6. `issues.description_format`과 `issue_comments.body_format`은 Markdown/HTML 원문 해석 방식을 저장한다. HTML로 작성된 본문은 HTML 내보내기에서 원문을 보존하고, Markdown으로 작성된 본문은 HTML 내보내기 시 안전하게 변환한다.
7. SQLite를 기본 저장소로 사용할 경우, 데이터 파일은 데스크톱 애플리케이션이 일관되게 관리하도록 설계하고 WAL 모드와 외래 키 제약을 기본 전제로 둔다.
8. 첨부 파일은 DB 파일 복사만으로 동기화가 가능하도록 기본적으로 SQLite 내부 저장 또는 동등한 단일 파일 포터블 구조를 우선 고려한다.
9. 검색은 SQLite FTS5와 적절한 인덱스를 우선 활용하며, 이후 필요 시 별도 인덱스 계층을 붙일 수 있도록 분리한다.
10. 내보내기 preset은 범위, 포맷, 첨부 포함 여부, 활동 로그 포함 여부 같은 세부 옵션을 저장할 수 있어야 한다.
...
5. 이슈 설명과 댓글의 읽기 렌더링은 Avalonia native WebView 경로를 사용한다. Markdown은 HTML로 변환한 뒤 동일 경로로 렌더링하고, HTML은 사용자가 작성한 CSS를 보존하기 위해 가능한 한 원문 구조를 유지한다. 헤드리스 테스트는 같은 ViewModel 출력을 텍스트 fallback으로 검증한다.
6. 대량 이슈 목록과 프로젝트 테이블을 고려해 가상화, 키보드 단축키, 커맨드 팔레트, 고속 필터링을 1급 요구사항으로 둔다.
...
이슈 CRUD, 댓글, 상태 변경, 닫힘 사유 관리, 라벨 연결, 담당자 연결, 이슈 타입, due date를 포함한 메타데이터 변경, 이력 기록을 담당한다. 이 모듈은 이슈 설명과 댓글의 원문 포맷을 `IssueContentFormat`으로 명시해 Markdown/HTML 작성 흐름이 UI, 저장소, 내보내기 사이에서 일관되게 유지되도록 한다. 로컬 앱의 읽기 화면은 이 원문 포맷을 바탕으로 WebView용 HTML 문서를 생성하며, HTML CSS 재현성을 사용자 경험의 일부로 취급한다.
...
테마는 1차 구현 기준으로 `WhiteBlue`, `BlueOrange`, `DarkBlue`, `DarkOrange` 네 가지 명시적 팔레트를 제공하며, 저장소에는 하위 호환을 고려한 직렬화 문자열로 보존한다. 기존 `light`, `system`, `dark` 값이 남아 있는 워크스페이스는 각각 신규 기본 라이트 계열 또는 다크 블루 계열로 안전하게 승격해 사용자가 업그레이드 후에도 설정 손실을 겪지 않게 한다.

### 16.8 Export / Portability Module

선택형 공유/내보내기, 포맷 변환, preset 저장, DB 파일 기반 portability 규칙을 담당한다. 내보내기 모듈은 출력 포맷뿐 아니라 본문 원본 포맷도 함께 고려해 Markdown 본문은 HTML 출력에서 변환하고, HTML 본문은 사용자가 작성한 원문 구조를 보존한다.
...
8. 이슈 목록과 이슈 상세는 하나의 단일 프레임 안에서 동시에 처리하지 않고, 목록 행 선택 시 상세 화면으로 이동하는 GitHub Issues식 흐름을 기본 상호작용으로 확정한다.
9. 앱 테마는 화이트-파랑, 파랑-오렌지, 다크-파랑, 다크-오렌지 네 가지를 1차 지원 범위로 확정한다.
10. 이슈 설명과 댓글은 Markdown과 HTML 두 작성 포맷을 모두 지원하며, 전역 우측 패널이 아니라 이슈 상세 화면 내부에서 포맷 선택, 본문 수정, 댓글 작성, 메타데이터 저장이 이어지도록 확정한다.
11. 로컬에서 사용하는 앱이라는 현재 조건에서는 HTML/CSS 렌더링의 충실도를 우선한다. 따라서 실제 앱은 WebView 기반 렌더링을 기본으로 삼고, 안전한 텍스트 블록 변환은 테스트와 fallback 용도로만 유지한다.

## 27. 결론

Tracky의 설계 핵심은 "GitHub를 닮은 익숙한 레이아웃" 그 자체가 아니라, GitHub 사용자에게 자연스럽고 빠르게 이해되는 작업 흐름과 GitHub Issues의 세부 도메인 기능을 개인 개발자용 standalone 제품 안에서 재구성하는 데 있다. 따라서 이 문서는 화면을 단순 모사하는 방향보다, All Issues 중심 홈 화면, 목록에서 상세로 이동하는 이슈 상세 화면, 상세 내부 메타데이터 편집, 프로젝트 보드/테이블/캘린더/타임라인, 검색 및 필터, 활동 로그, 상태 사유 모델, due date와 리마인더, Markdown/HTML 본문 포맷, 첨부 파일, DB 파일 기반 portability, 세밀한 공유/내보내기, 크로스 플랫폼 UX, 정돈된 시각 체계 같은 핵심 경험을 중심으로 제품 구조를 정의했다.
